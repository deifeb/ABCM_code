...
def format_trans(data):
    ...
    date_range = pd.date_range(start=start_date, periods=51, freq='M')  # 从开始日期生成84个月的时间序列
    ...
    for i, col in enumerate(data.columns):
        if col != 'ds':
            ...
            Qtrain_train_series = {
                "start": data['ds'].iloc[0],
                "target": data[col].iloc[:48].values.tolist(),  # 仅取前78行作为target
                "feat_static_cat": [i - 1],
            }
    ...
    for col in data.columns:
        df = pd.DataFrame(
            {'time': pd.date_range(start='1996-01-01', periods=51, freq='M'), col: data[col]})
        tss.append(df)
    if len(tss) > 0:
        del tss[0]
    for dff in tss:
...
def calculate_model_acc(action, train_data, test_data, tss):
    ...
    if action in function_mapping:
        if action == 'Croston' or action == 'SBA' or action == 'SBJ' or action == 'ARIMA' or action == 'ETS':
            metric = function_mapping[action](train_data, test_data, tss, 'M', 3, evaluator)
            return metric
        elif action == 'DeepRenewal Flat' or action == 'DeepRenewal Exact' or action == 'DeepRenewal Hybrid':
            metric = function_mapping[action](train_data, test_data, tss, 3, 0.01, 'M', 10, 'lstm',
                                              64, 0.3, 1, evaluator, action)
            return metric
        elif action == 'DeepAR':
            metric = function_mapping[action](train_data, test_data, tss, 3, 0.01, 'M',
...
if __name__ == '__main__':
    ...
    labels['category'] = pd.read_excel(r"E:\论文论文\迁移学习\程序\特征提取\美国汽车零部件2674_y1.xlsx", index_col=0)
    ...
    errors.to_excel(r'E:\gluon_ts\新聚类方法\美国汽车零部件2674预测结果.xlsx')
