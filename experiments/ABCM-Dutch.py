...
def format_trans(data):
    ...
    date_range = pd.date_range(start=start_date, periods=150, freq='M')  # 从开始日期生成84个月的时间序列
    ...
    for col in data.columns:
        df = pd.DataFrame(
            {'time': pd.date_range(start='1996-01-01', periods=150, freq='M'), col: data[col]})
        tss.append(df)
    if len(tss) > 0:
        del tss[0]
    for dff in tss:
...
def calculate_model_acc(action, train_data, test_data, tss):
    ...
    if action in function_mapping:
        if action == 'Croston' or action == 'SBA' or action == 'SBJ' or action == 'ARIMA' or action == 'ETS':
            metric = function_mapping[action](train_data, test_data, tss, 'M', 12, evaluator)
            return metric
        elif action == 'DeepRenewal Flat' or action == 'DeepRenewal Exact' or action == 'DeepRenewal Hybrid':
            metric = function_mapping[action](train_data, test_data, tss, 12, 0.01, 'M', 10, 'lstm',
                                              64, 0.3, 1, evaluator, action)
            return metric
        elif action == 'DeepAR':
            metric = function_mapping[action](train_data, test_data, tss, 12, 0.01, 'M',
...
if __name__ == '__main__':
    # df = pd.read_excel(r"E:\论文论文\迁移学习\程序\特征提取\美国汽车零部件2674非0少5.xlsx", index_col=0)
    df = pd.read_excel(r"E:\数据集\荷兰制造公司3451件商品销售情况\manufacturing_dataset_non_zero_filtered.xlsx", index_col=0)
    labels = pd.DataFrame()
    data_names = df.columns.tolist()
    labels['data_names'] = data_names
    labels['category'] = pd.read_excel(r"E:\论文论文\迁移学习\程序\特征提取\荷兰零部件y1.xlsx", index_col=0)
    ...
    q_table = pd.read_excel(r'q_tables_xin2.xlsx', index_col=0)
    for i in range(grouped_data.shape[0]):
        b_data = df[grouped_data['data_names'][i]]
        train_data_a, test_data_a, tss_a = format_trans(b_data)
        model = q_table[row_index[i]].idxmax()
        print(model)
        metric = calculate_model_acc(model, train_data_a, test_data_a, tss_a)
        error.append(metric)
    errors = pd.DataFrame(error)
    errors.to_excel(r'E:\gluon_ts\新聚类方法\荷兰零部件预测结果.xlsx')
